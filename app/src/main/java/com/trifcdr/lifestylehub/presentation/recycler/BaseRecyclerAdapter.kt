package com.trifcdr.lifestylehub.presentation.recycler
...
class BaseRecyclerAdapter(
    ...
    override fun getItemViewType(position: Int): Int = vHoldersManager.getItemType(getItem(position))
...
class BaseDiffCallback : DiffUtil.ItemCallback<StringId>() {
    override fun areItemsTheSame(oldItem: StringId, newItem: StringId): Boolean = oldItem.id == newItem.id
    override fun areContentsTheSame(oldItem: StringId, newItem: StringId): Boolean = oldItem == newItem
}